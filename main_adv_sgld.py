...
from models.sgld import SGLDM, SGLD
...
parser.add_argument('--optim', default='sgld', type=str)
parser.add_argument('--noise-std', default=1.0, type=float)
parser.add_argument('--resume', action='store_true', help='resume')
parser.add_argument('--noise-type', default='normal', type=str)

opt = parser.parse_args()

from datetime import datetime
now=datetime.now()
#log_file = 'checkpoint/%s_adv_%s_%f_%s.log' % (opt.data,opt.optim,opt.noise_std,now.strftime("%m-%d-%Y-%H-%M-%S"))
log_file = 'checkpoint/%s_adv_%s_%s_%f.log' % (opt.data,opt.optim,opt.noise_type, opt.noise_std)
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        total += targets.numel()
    logging.info(f'[TRAIN] Acc: {100.*correct/total:.3f}, Loss: {loss.data:.3f}')
...
def test(epoch):
    ...
    # Save checkpoint.
    torch.save({'state_dict': net.state_dict(), 'epoch': epoch, 'lr': opt.lr}, opt.model_out)
...
for epoch in epochs:
    if opt.optim == 'sgld':
        if opt.noise_std == 0:
            optimizer = SGLD(net.parameters(), lr=opt.lr, weight_decay=5.0e-4, noise_std=opt.noise_std, addnoise=False, noise_type=opt.noise_type)
        else:
            optimizer = SGLD(net.parameters(), lr=opt.lr, weight_decay=5.0e-4, noise_std=opt.noise_std, addnoise=True, noise_type=opt.noise_type)
            
    elif opt.optim == 'sgldm':
        optimizer = SGLDM(net.parameters(), lr=opt.lr, noise_std=opt.noise_std, momentum=0.9, weight_decay=5.0e-4, addnoise=True, noise_type=opt.noise_type)
    
    elif opt.optim == 'sgd':
        optimizer = SGD(net.parameters(), lr=opt.lr, momentum=0.9, weight_decay=5.0e-4)
        
    elif opt.optim == 'psgld':
        optimizer = pSGLD(net.parameters(), lr=opt.lr, norm_sigma=opt.norm_sigma,noise_std=opt.noise_std, addnoise=True)
    ...
    os.system('cp ' + opt.model_out + ' ' + opt.model_out[:-4]+'_'+str(opt.lr)+'.pth')
    opt.lr /= 10
