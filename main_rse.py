...
parser.add_argument('--noise_init', required=True, type=float, help='Init noise std')
parser.add_argument('--noise_inner', required=True, type=float, help='Inner noise std')
...
now=datetime.now()
log_file = 'checkpoint/%s_rse_%s.log' % (opt.data, now.strftime("%m-%d-%Y-%H-%M-%S"))
...
if opt.model == 'vgg':
    ...
    net = nn.DataParallel(VGG('VGG16', nclass, opt.noise_init, opt.noise_inner, img_width=img_width).cuda())
elif opt.model == 'aaron':
    from models.aaron_rse import Aaron
    net = nn.DataParallel(Aaron(nclass, opt.noise_init, opt.noise_inner).cuda())
else:
